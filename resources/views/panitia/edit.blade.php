            ...
            <form action="{{ route('panitia.update', $panitia->id) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                    ...
                    <div>
                        ...
                        <select name="jabatan" id="jabatan" class="input-field">
                            <option value="">Pilih Jabatan</option>
                            @foreach($jabatanOptions as $option)
                                <option value="{{ $option }}" {{ old('jabatan', $panitia->jabatan) == $option ? 'selected' : '' }}>{{ $option }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="prodi" id="prodi" class="input-field">
                            ...
                            @foreach($prodiOptions as $option)
                                <option value="{{ $option }}" {{ old('prodi', $panitia->prodi) == $option ? 'selected' : '' }}>{{ $option }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="kelas" id="kelas" class="input-field">
                            ...
                            @foreach($kelasOptions as $option)
                                <option value="{{ $option }}" {{ old('kelas', $panitia->kelas) == $option ? 'selected' : '' }}>{{ $option }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="tingkat" id="tingkat" class="input-field">
                            ...
                            @foreach($tingkatOptions as $option)
                                <option value="{{ $option }}" {{ old('tingkat', $panitia->tingkat) == $option ? 'selected' : '' }}>{{ $option }}</option>
                            @endforeach
                        </select>
                    </div>
